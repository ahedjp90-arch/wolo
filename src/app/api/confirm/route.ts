...
export async function POST(request: Request) {
  ...
  await supabase.from('confirmations').insert([{ user_id: userId, token }]);
  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto; background: #0F0F1A; color: #E8E8F0; padding: 40px; border-radius: 16px;">
        ...
        <p style="color: #B8B8D0; line-height: 1.6;">Merci de vous être inscrit sur WOLO. Cliquez sur le bouton ci-dessous pour confirmer votre adresse email et accéder à votre compte.</p>
        ...
        <p style="color: #4A4A6A; font-size: 12px;">Si vous n'avez pas créé de compte WOLO, ignorez cet email.</p>
...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabase.from('confirmations').select('*').eq('token', token).single();

  if (error || !data) return NextResponse.json({ error: 'Token invalide' }, { status: 400 });
  if (data.confirmed) return NextResponse.json({ error: 'Déjà confirmé' }, { status: 400 });
