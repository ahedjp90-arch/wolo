      ...
      <div style={{ minHeight: "100vh", background: bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Inter, sans-serif" }}>
        <div style={{ width: 360, background: card, border: `1px solid ${border}`, borderRadius: 16, padding: 40 }}>
          ...
          <input type="password" placeholder="Mot de passe admin" value={password} onChange={e => setPassword(e.target.value)}
            ...
            style={{ width: "100%", background: isDark ? "#0F0F1A" : "#F9FAFB", border: `1px solid ${border}`, borderRadius: 8, padding: "12px 14px", color: text, fontSize: 13, outline: "none", boxSizing: "border-box" }} />
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 20 }}>
            <button onClick={() => setTheme("light")} style={{ background: theme === "light" ? "#F5A623" : "transparent", border: `1px solid ${border}`, borderRadius: 6, color: theme === "light" ? "#0F0F1A" : sub, padding: "6px 12px", fontSize: 12, cursor: "pointer" }}>☀️ Clair</button>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => setTheme("light")} style={{ background: theme === "light" ? "#F5A623" : "transparent", border: `1px solid ${border}`, borderRadius: 6, color: theme === "light" ? "#0F0F1A" : sub, padding: "6px 12px", fontSize: 12, cursor: "pointer" }}>☀️</button>
            <button onClick={() => setTheme("dark")} style={{ background: theme === "dark" ? "#F5A623" : "transparent", border: `1px solid ${border}`, borderRadius: 6, color: theme === "dark" ? "#0F0F1A" : sub, padding: "6px 12px", fontSize: 12, cursor: "pointer" }}>🌙</button>
          </div>
        </div>
          ...
          {!loading && (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                ...
                {users.length === 0 && (
                  <tr><td colSpan={6} style={{ padding: 40, textAlign: "center", color: sub }}>Aucun utilisateur actif</td></tr>
                )}
              </tbody>
            </table>
          )}
