...
export default function WoloAdmin() {
  ...
  const [stats, setStats] = useState({ totalUsers: 0, totalClients: 0, totalTransactions: 0 });
  ...
  const fetchData = async () => {
    ...
    const [clientsRes, transactionsRes, tachesRes, wikiRes] = await Promise.all([
      ...
      supabaseAdmin.from("transactions").select("user_id, montant, type"),
      supabaseAdmin.from("taches").select("user_id"),
      supabaseAdmin.from("wiki").select("user_id"),
    ]);
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 32 }}>
          {[
            ...
            { label: "Revenus totaux", valeur: stats.totalRevenu?.toLocaleString() + " FCFA", icon: "📈" },
          ].map((s, i) => (
