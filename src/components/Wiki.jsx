   ...
   <div style={{ flex: 1, overflow: "auto", padding: "28px 32px", background: "#0F0F1A" }}>
     ...
     {!loading && (
       <div style={{ display: "grid", gridTemplateColumns: selected ? "1fr 1.5fr" : "repeat(3, 1fr)", gap: 16 }}>
         {filtered.length === 0 && !selected && <div style={{ gridColumn: "span 3", color: "#3A3A5A", textAlign: "center", padding: 40, fontSize: 14 }}>Aucun article</div>}
         {filtered.map(article => (
           <div key={article.id} onClick={() => setSelected(selected?.id === article.id ? null : article)}
             ...
             <div style={{ fontSize: 14, fontWeight: 600, color: "#E8E8F0", marginBottom: 8 }}>{article.titre}</div>
         ...
         {selected && (
           <div style={{ background: "#111128", border: "1px solid #1E1E38", borderRadius: 14, padding: 24 }}>
             ...
             <div style={{ fontSize: 18, fontWeight: 700, color: "#E8E8F0", marginBottom: 16 }}>{selected.titre}</div>
