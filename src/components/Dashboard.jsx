  ...
  return (
    <div style={{ flex: 1, overflow: "auto", padding: "28px 32px", background: "#0F0F1A" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 22, fontWeight: 700, color: "#E8E8F0" }}>Bonjour, Jhon 👋</div>
        <div style={{ fontSize: 13, color: "#6B6B8A", marginTop: 2 }}>3 tâches urgentes aujourd'hui</div>
      </div>

      {/* KPIs */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
        {kpis.map((k, i) => (
          <div key={i} style={{ background: "#111128", border: "1px solid #1E1E38", borderRadius: 14, padding: "20px 22px" }}>
            <div style={{ fontSize: 12, color: "#6B6B8A", marginBottom: 10, textTransform: "uppercase", letterSpacing: "0.05em" }}>{k.label}</div>
            <div style={{ fontSize: 26, fontWeight: 700, color: "#E8E8F0" }}>{k.valeur} <span style={{ fontSize: 12, color: "#6B6B8A", fontWeight: 400 }}>{k.unite}</span></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 20, marginBottom: 24 }}>
        <div style={{ background: "#111128", border: "1px solid #1E1E38", borderRadius: 14, padding: "22px 24px" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: "#E8E8F0", marginBottom: 4 }}>Revenus & Dépenses</div>
          <div style={{ fontSize: 12, color: "#6B6B8A", marginBottom: 16 }}>6 derniers mois</div>
          <ResponsiveContainer width="100%" height={180}>
            <AreaChart data={revenueData}>
              ...
              <XAxis dataKey="mois" tick={{ fill: "#6B6B8A", fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ background: "#1A1A2E", border: "1px solid #2A2A45", borderRadius: 8, color: "#E8E8F0", fontSize: 12 }} />
        ...
        <div style={{ background: "#111128", border: "1px solid #1E1E38", borderRadius: 14, padding: "22px 24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: "#E8E8F0" }}>Tâches du jour</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {taches.map(task => (
              <div key={task.id} onClick={() => toggle(task.id)} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "10px 12px", borderRadius: 10, background: task.fait ? "rgba(74,155,142,0.06)" : "rgba(255,255,255,0.02)", border: `1px solid ${task.fait ? "rgba(74,155,142,0.15)" : "#1E1E38"}`, cursor: "pointer" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, color: task.fait ? "#4B4B6A" : "#E8E8F0", textDecoration: task.fait ? "line-through" : "none" }}>{task.titre}</div>
                  <div style={{ fontSize: 11, color: "#6B6B8A", marginTop: 2 }}>{task.heure}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 20 }}>
        <div style={{ background: "#111128", border: "1px solid #1E1E38", borderRadius: 14, padding: "22px 24px" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: "#E8E8F0", marginBottom: 16 }}>Clients actifs</div>
          {clients.map((c, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 10px", borderBottom: i < clients.length - 1 ? "1px solid #1A1A30" : "none" }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#E8E8F0" }}>{c.nom}</div>
                <div style={{ fontSize: 11, color: "#6B6B8A" }}>{c.valeur}</div>
              </div>
        ...
        <div style={{ background: "#111128", border: "1px solid #1E1E38", borderRadius: 14, padding: "22px 24px" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: "#E8E8F0", marginBottom: 16 }}>Alertes récentes</div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {alertes.map((a, i) => (
              <div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: alerteColors[a.type].bg, borderLeft: `3px solid ${alerteColors[a.type].border}` }}>
                <div style={{ fontSize: 12, color: "#E8E8F0", lineHeight: 1.4 }}>{a.message}</div>
                <div style={{ fontSize: 11, color: "#6B6B8A", marginTop: 4 }}>{a.temps}</div>
              </div>
            ))}
          </div>
        </div>
