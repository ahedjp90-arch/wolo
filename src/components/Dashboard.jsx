...
export default function Dashboard({ theme }) {
  ...
  const tachesAFaire = taches.filter(t => t.colonne === "A faire").length;
  const facturesImpayees = factures.filter(f => f.statut === "Impayee").length;

  // Graphique mensuel basé sur les vraies données
  const currentYear = new Date().getFullYear();
  const graphData = moisLabels.map((mois, i) => {
    const revenus = transactions.filter(t => {
      if (!t.created_at) return false;
      const d = new Date(t.created_at);
      return t.type === "entree" && d.getMonth() === i && d.getFullYear() === currentYear;
    }).reduce((s, t) => s + t.montant, 0);

    const depenses = transactions.filter(t => {
      if (!t.created_at) return false;
      const d = new Date(t.created_at);
      return t.type === "sortie" && d.getMonth() === i && d.getFullYear() === currentYear;
    }).reduce((s, t) => s + t.montant, 0);
  ...
  const clientsParStatut = [
    { statut: "Prospect", count: clients.filter(c => c.statut === "Prospect").length, color: "#6B6B8A" },
    { statut: "Negotiation", count: clients.filter(c => c.statut === "Negociation").length, color: "#F5A623" },
    { statut: "Gagne", count: clients.filter(c => c.statut === "Gagne").length, color: "#4A9B8E" },
    { statut: "Relance", count: clients.filter(c => c.statut === "Relance due").length, color: "#E85555" },
  ];
  ...
  return (
    <div style={{ flex: 1, overflow: "auto", padding: "28px 32px", background: bg }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ fontSize: 13, color: sub, marginTop: 2 }}>{tachesAFaire} taches a faire · {facturesImpayees} facture{facturesImpayees > 1 ? "s" : ""} impayee{facturesImpayees > 1 ? "s" : ""}</div>
      </div>

      {/* KPIs */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
        {[
          { label: "Revenus", valeur: totalEntrees.toLocaleString("fr-FR"), unite: "FCFA", color: "#4A9B8E", icon: "▲" },
          { label: "Depenses", valeur: totalSorties.toLocaleString("fr-FR"), unite: "FCFA", color: "#E85555", icon: "▼" },
          { label: "Solde net", valeur: (totalEntrees - totalSorties).toLocaleString("fr-FR"), unite: "FCFA", color: totalEntrees >= totalSorties ? "#4A9B8E" : "#E85555", icon: "=" },
          { label: "Clients", valeur: clients.length.toString(), unite: "clients", color: "#7C7CF0", icon: "👥" },
        ].map((k, i) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 20, marginBottom: 24 }}>
        <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 14, padding: "22px 24px" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: text, marginBottom: 4 }}>Revenus & Depenses {currentYear}</div>
          <div style={{ fontSize: 12, color: sub, marginBottom: 16 }}>Donnees reelles par mois</div>
            ...
            <AreaChart data={graphData}>
              <defs>
                ...
                <linearGradient id="g2" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#E85555" stopOpacity={0.2} />
                  <stop offset="95%" stopColor="#E85555" stopOpacity={0} />
                </linearGradient>
              </defs>
              ...
              <Area type="monotone" dataKey="revenus" name="Revenus" stroke="#F5A623" strokeWidth={2} fill="url(#g1)" dot={false} />
              <Area type="monotone" dataKey="depenses" name="Depenses" stroke="#E85555" strokeWidth={2} fill="url(#g2)" dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        {/* Clients par statut */}
        <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 14, padding: "22px 24px" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: text, marginBottom: 16 }}>Pipeline clients</div>
          <ResponsiveContainer width="100%" height={120}>
            <BarChart data={clientsParStatut}>
              <XAxis dataKey="statut" tick={{ fill: sub, fontSize: 10 }} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={{ background: card, border: `1px solid ${border}`, borderRadius: 8, color: text, fontSize: 12 }} />
              <Bar dataKey="count" name="Clients" fill="#F5A623" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
      ...
      {/* Clients recents + Transactions */}
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 20 }}>
        <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 14, padding: "22px 24px" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: text, marginBottom: 16 }}>Clients recents</div>
        ...
        <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 14, padding: "22px 24px" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: text, marginBottom: 16 }}>Transactions recentes</div>
