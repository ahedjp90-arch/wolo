...
export default function Facturation({ theme }) {
 ...
 const exportPDF = async (facture) => {
   ...
   doc.setFontSize(11);
   doc.setFont("helvetica", "bold");
   doc.text("Facture a :", 14, 60);
   doc.setFont("helvetica", "normal");
   doc.setFontSize(10);
   doc.text(facture.client || "Client", 14, 68);
   if (facture.echeance) doc.text(`Echeance : ${facture.echeance}`, 14, 76);
   ...
   autoTable(doc, {
     startY: 90,
     head: [["Description", "Qte", "Prix Unit. (FCFA)", "Total (FCFA)"]],
     body: facture.lignes.map(l => [
       l.description,
       l.quantite,
       formatNum(l.prix),
       formatNum(l.quantite * l.prix)
     ]),
   ...
   doc.setFontSize(10);
   doc.setTextColor(0, 0, 0);
   doc.text(`Total HT : ${formatNum(facture.totalHT)} FCFA`, 140, finalY);
   doc.text(`TVA (18%) : ${formatNum(facture.tva)} FCFA`, 140, finalY + 8);
   doc.setFont("helvetica", "bold");
   doc.setFontSize(12);
   doc.setTextColor(245, 166, 35);
   doc.text(`Total TTC : ${formatNum(facture.totalTTC)} FCFA`, 140, finalY + 18);
   ...
   <div style={{ flex: 1, overflow: "auto", padding: "28px 32px", background: bg }}>
     <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
       <div>
         ...
         <div style={{ fontSize: 13, color: sub, marginTop: 2 }}>{factures.length} facture{factures.length > 1 ? "s" : ""}</div>
       </div>
     ...
     {showForm && (
       <div style={{ background: card, border: `1px solid ${inputBorder}`, borderRadius: 14, padding: 28, marginBottom: 24 }}>
         ...
         <div style={{ background: isDark ? "#0F0F1A" : "#F9FAFB", borderRadius: 10, padding: "14px 20px", marginBottom: 16, textAlign: "right" }}>
           <div style={{ fontSize: 13, color: sub, marginBottom: 4 }}>Total HT : {formatNum(totalLignes)} FCFA</div>
           <div style={{ fontSize: 13, color: sub, marginBottom: 4 }}>TVA (18%) : {formatNum(tva)} FCFA</div>
           <div style={{ fontSize: 16, fontWeight: 700, color: "#F5A623" }}>Total TTC : {formatNum(totalTTC)} FCFA</div>
         </div>

         <textarea placeholder="Notes (optionnel)" value={newFacture.notes} onChange={e => setNewFacture({ ...newFacture, notes: e.target.value })}
     ...
     <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
       ...
       {factures.map(f => (
         <div key={f.id} style={{ background: card, border: `1px solid ${border}`, borderRadius: 14, padding: "20px 24px", display: "flex", alignItems: "center", gap: 16 }}>
           ...
           <div style={{ fontSize: 18, fontWeight: 700, color: "#F5A623", marginRight: 16 }}>{formatNum(f.totalTTC)} FCFA</div>
