...
export default function Taches() {
 ...
 const [newTache, setNewTache] = useState({ titre: "", priorite: "moyenne", categorie: "CRM", colonne: "À faire" });
 ...
 const addTache = async () => {
   ...
   if (!error) { fetchTaches(); setNewTache({ titre: "", priorite: "moyenne", categorie: "CRM", colonne: "À faire" }); setShowForm(false); }
 };
       ...
       <div style={{ background: "#111128", border: "1px solid #2A2A45", borderRadius: 14, padding: 24, marginBottom: 24 }}>
         ...
         <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
           <div style={{ gridColumn: "span 2", display: "flex", flexDirection: "column", gap: 4 }}>
             <label style={{ fontSize: 11, color: "#6B6B8A", textTransform: "uppercase", letterSpacing: "0.05em" }}>Titre</label>
           ...
           <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
             <label style={{ fontSize: 11, color: "#6B6B8A", textTransform: "uppercase", letterSpacing: "0.05em" }}>Priorité</label>
           ...
           <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
             <label style={{ fontSize: 11, color: "#6B6B8A", textTransform: "uppercase", letterSpacing: "0.05em" }}>Colonne</label>
               ...
               {taches.filter(t => t.colonne === colonne).map(task => (
                 <div key={task.id} style={{ background: "#0F0F1A", border: "1px solid #1E1E38", borderRadius: 10, padding: 14, borderLeft: `3px solid ${prioriteColors[task.priorite]}` }}>
                   ...
                   <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                     <span style={{ fontSize: 11, color: "#6B6B8A", background: "#1A1A2E", padding: "2px 8px", borderRadius: 10 }}>{task.categorie}</span>
