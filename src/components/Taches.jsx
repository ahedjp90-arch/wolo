...
export default function Taches() {
  ...
  const fetchTaches = async () => {
    setLoading(true);
    const { data, error } = await supabase.from("taches").select("*").order("created_at", { ascending: false });
  ...
  const addTache = async () => {
    ...
    const { error } = await supabase.from("taches").insert([{ ...newTache, user_id: user.id }]);
