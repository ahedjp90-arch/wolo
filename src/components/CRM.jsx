...
export default function CRM({ theme }) {
 ...
 const updateClient = async () => {
   if (!editClient || !userId) return;
   const { error } = await supabase.from("clients").update({
     nom: editClient.nom, contact: editClient.contact, telephone: editClient.telephone,
     email: editClient.email, secteur: editClient.secteur, statut: editClient.statut,
     valeur: editClient.valeur, pipeline: editClient.pipeline, notes: editClient.notes
   }).eq("id", editClient.id).eq("user_id", userId);
   if (!error) {
     fetchClients(userId);
     ...
     {showForm && (
       <div style={{ background: card, border: `1px solid ${inputBorder}`, borderRadius: 14, padding: 24, marginBottom: 24 }}>
         ...
         <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
           ...
           <input placeholder="Budget estimé (FCFA)" value={newClient.valeur} onChange={e => setNewClient({ ...newClient, valeur: e.target.value })} style={inputStyle} />
     ...
     {!loading && (
       <div style={{ display: "grid", gridTemplateColumns: selected ? "1fr 1fr" : "1fr", gap: 20 }}>
         <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 14, overflow: "hidden" }}>
           ...
           {filtered.map((c, i) => (
             <div key={c.id} onClick={() => { setSelected(selected?.id === c.id ? null : c); setEditMode(false); }}
         ...
         {selected && !editMode && (
           <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 14, padding: 24 }}>
             ...
             {[["Contact", selected.contact], ["Téléphone", selected.telephone], ["Email", selected.email], ["Secteur", selected.secteur], ["Budget", selected.valeur ? selected.valeur + " FCFA" : "–"], ["Statut", selected.statut]].map(([label, val]) => (
             ...
             <div style={{ display: "flex", gap: 8 }}>
               <button onClick={() => { setEditClient({ ...selected }); setEditMode(true); }} style={{ flex: 1, background: "linear-gradient(135deg, #F5A623, #E8830A)", border: "none", borderRadius: 8, color: "#0F0F1A", padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>✏️ Modifier</button>
               <button onClick={() => deleteClient(selected.id)} style={{ flex: 1, background: "rgba(232,85,85,0.1)", border: "none", borderRadius: 8, color: "#E85555", padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Supprimer</button>
             </div>
           </div>
         )}

         {selected && editMode && editClient && (
           <div style={{ background: card, border: `1px solid ${border}`, borderRadius: 14, padding: 24 }}>
             <div style={{ fontSize: 15, fontWeight: 600, color: text, marginBottom: 16 }}>Modifier le client</div>
             <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
               <input placeholder="Nom entreprise" value={editClient.nom} onChange={e => setEditClient({ ...editClient, nom: e.target.value })} style={inputStyle} />
               <input placeholder="Nom contact" value={editClient.contact} onChange={e => setEditClient({ ...editClient, contact: e.target.value })} style={inputStyle} />
               <input placeholder="Téléphone" value={editClient.telephone} onChange={e => setEditClient({ ...editClient, telephone: e.target.value })} style={inputStyle} />
               <input placeholder="Email" value={editClient.email} onChange={e => setEditClient({ ...editClient, email: e.target.value })} style={inputStyle} />
               <select value={editClient.secteur} onChange={e => setEditClient({ ...editClient, secteur: e.target.value })} style={inputStyle}>
                 <option value="">Choisir un secteur</option>
                 {secteurs.map(s => <option key={s} value={s}>{s}</option>)}
               </select>
               <input placeholder="Budget estimé (FCFA)" value={editClient.valeur} onChange={e => setEditClient({ ...editClient, valeur: e.target.value })} style={inputStyle} />
               <select value={editClient.statut} onChange={e => setEditClient({ ...editClient, statut: e.target.value })} style={inputStyle}>
                 <option value="Prospect">Prospect</option>
                 <option value="Négociation">Négociation</option>
                 <option value="Gagné">Gagné</option>
                 <option value="Relance due">Relance due</option>
               </select>
               <select value={editClient.pipeline} onChange={e => setEditClient({ ...editClient, pipeline: e.target.value })} style={inputStyle}>
                 <option value="Premier contact">Premier contact</option>
                 <option value="Proposition envoyée">Proposition envoyée</option>
                 <option value="En négociation">En négociation</option>
                 <option value="Contrat signé">Contrat signé</option>
               ...
               <textarea placeholder="Notes..." value={editClient.notes} onChange={e => setEditClient({ ...editClient, notes: e.target.value })}
                 style={{ ...inputStyle, resize: "vertical", minHeight: 80 }} />
             </div>
             <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
               <button onClick={updateClient} style={{ flex: 1, background: "linear-gradient(135deg, #F5A623, #E8830A)", border: "none", borderRadius: 8, color: "#0F0F1A", padding: "10px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>✅ Sauvegarder</button>
               <button onClick={() => setEditMode(false)} style={{ flex: 1, background: "transparent", border: `1px solid ${inputBorder}`, borderRadius: 8, color: sub, padding: "10px", fontSize: 13, cursor: "pointer" }}>Annuler</button>
             </div>
           </div>
         )}
       </div>
